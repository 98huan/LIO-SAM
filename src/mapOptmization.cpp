...
public:
    ...
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr &msgIn)
    {
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)
        {
            ...
            // 当前帧位姿初始化
            // 1、如果是第一帧，用原始imu数据的RPY初始化当前帧位姿（旋转部分）
            // 2、后续帧，用imu里程计计算两帧之间的增量位姿变换，作用于前一帧的激光位姿，得到当前
            updateInitialGuess();
    ...
    /**
     * 激光雷达坐标系下的激光点，通过激光帧位姿，变换到世界坐标系下
     */
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void cornerOptimization()
    {
        ...
        for (int i = 0; i < laserCloudCornerLastDSNum; i++)
        {
            ...
            if (pointSearchSqDis[4] < 1.0)
            {
                ...
                // 构建协方差矩阵
                matA1.at<float>(0, 0) = a11; matA1.at<float>(1, 0) = a12; matA1.at<float>(2, 0) = a13;
                matA1.at<float>(0, 1) = a12; matA1.at<float>(1, 1) = a22; matA1.at<float>(2, 1) = a23;
                matA1.at<float>(0, 2) = a13; matA1.at<float>(1, 2) = a23; matA1.at<float>(2, 2) = a33;
    ...
    */
    void updatePath(const PointTypePose &pose_in)
    {
        ...
        // TODO:位姿写入txt
        // 位姿输出到txt文档
        // std::string lio_sam_path = ros::package::getPath("lio_sam"); //会报错
